use crate::constants::{
    EMPTY_NIBBLES, HIGH_NIBBLE_MASK, NUM_BITS_IN_NIBBLE, NUM_NIBBLES_IN_BYTE, ZERO_BYTE,
};
...
pub fn get_common_prefix_nibbles(
    ...
    let longer_nibbles = if a_is_shorter { nibbles_b } else { nibbles_a };
    let (common_prefix, a, b) =
        get_common_prefix_nibbles_recursively(shorter_nibbles, longer_nibbles, EMPTY_NIBBLES)?;
    match a_is_shorter {
...
fn get_common_prefix_nibbles_recursively(
    ...
    match get_length_in_nibbles(&shorter_nibbles) {
        ...
        _ => {
            let first_nibble =
                get_nibbles_from_bytes(vec![get_nibble_at_index(&shorter_nibbles, 0)?]);
            match get_nibble_at_index(&shorter_nibbles, 0)?
                == get_nibble_at_index(&longer_nibbles, 0)?
            {
...
fn shift_bits_in_vec_right_one_nibble(bytes: Bytes) -> Bytes {
    match bytes.len() {
        ...
        _ => {
            ...
            let final_byte =
                merge_nibbles_from_bytes(ZERO_BYTE, bytes[bytes.len() - 1] << NUM_BITS_IN_NIBBLE);
...
fn remove_first_nibble(nibbles: Nibbles) -> Result<Nibbles> {
    match get_length_in_nibbles(&nibbles) {
        ...
        _ => match nibbles.offset {
            ...
            _ => replace_nibble_in_nibbles_at_nibble_index(nibbles, get_zero_nibble(), 0)
    ...
    match nibble_index {
        ...
        _ => {
            ...
            match (nibble_index + offset) % 2 == 0 {
                ...
                false => {
                    replace_nibble_in_nibbles_at_nibble_index(sliced_nibbles, get_zero_nibble(), 0)
                        .map(set_nibble_offset_to_one)
                }
            }
        }
...
mod tests {
    ...
    fn should_get_byte_containing_nibble_at_i_correctly() {
        ...
        let result =
            get_byte_containing_nibble_at_nibble_index(&nibbles, &index_of_nibble).unwrap();
        assert!(result == expected_result);
    }
    ...
    fn should_replace_low_nibble_in_byte_correctly() {
        ...
        let result =
            replace_low_nibble_in_byte(test_byte, get_nibbles_from_bytes(vec![replacement_nibble]));
        assert!(result == expected_result);
    }
    ...
    fn should_replace_byte_in_nibbles_correctly() {
        ...
        let updated_nibbles =
            replace_byte_in_nibbles_at_byte_index(byte_index, nibbles, replacement_byte);
    ...
    fn should_replace_offset_nibble_at_nibble_index_in_nibbles_correctly() {
        for nibble_index in 0..get_length_in_nibbles(&get_sample_nibbles()) {
            ...
            let byte_before =
                get_byte_containing_nibble_at_nibble_index(&nibbles_before, &nibble_index).unwrap();
            let replacement_nibble = get_nibbles_from_offset_bytes(
                vec![15u8], // [00001111]
            );
            let expected_byte = match nibble_index % 2 {
                0 => replace_high_nibble_in_byte(byte_before.clone(), replacement_nibble.clone()),
                _ => replace_low_nibble_in_byte(byte_before.clone(), replacement_nibble.clone()),
            };
            ...
            let byte_after =
                get_byte_containing_nibble_at_nibble_index(&nibbles_after, &nibble_index).unwrap();
    ...
    fn should_replace_offset_nibble_at_nibble_index_in_offset_nibbles_correctly() {
        for nibble_index in 0..get_length_in_nibbles(&get_sample_offset_nibbles()) {
            ...
            let expected_byte = match nibble_index % 2 {
                0 => replace_low_nibble_in_byte(byte_before.clone(), replacement_nibble.clone()),
                _ => replace_high_nibble_in_byte(byte_before.clone(), replacement_nibble.clone()),
            };
            let nibbles_after = replace_nibble_in_nibbles_at_nibble_index(
                nibbles_before.clone(),
                replacement_nibble.clone(),
    ...
    fn should_slice_nibbles_at_nibble_index_of_one_correctly() {
        ...
        let result = slice_nibbles_at_nibble_index(nibbles.clone(), nibble_index).unwrap();
        let expected_result =
            get_nibbles_from_offset_bytes(vec![0x2u8, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde]);
        assert!(result == expected_result);
    }
    ...
    fn should_remove_last_byte_from_offset_nibbles_correctly() {
        let nibbles = get_sample_offset_nibbles();
        let expected_result =
            get_nibbles_from_offset_bytes(vec![0x01u8, 0x23, 0x45, 0x67, 0x89, 0xab]);
        let result = remove_last_byte_from_nibbles(nibbles);
        assert!(result == expected_result);
    }
    ...
    fn should_get_common_prefix_nibbles_recursively_correctly_when_both_not_offset() {
        ...
        let (res_prefix, res_shorter, res_longer) =
            get_common_prefix_nibbles_recursively(shorter_nibbles, longer_nibbles, EMPTY_NIBBLES)
                .unwrap();
    ...
    fn get_common_prefix_nibbles_should_work_if_first_nibbles_are_shorter() {
        ...
        let expected_shorter_result = get_nibbles_from_bytes(shorter_bytes_suffix);
        let (res_prefix, res_1, res_2) =
            get_common_prefix_nibbles(shorter_nibbles, longer_nibbles).unwrap();
    ...
    fn get_common_prefix_nibbles_should_work_if_second_nibbles_are_shorter() {
        ...
        let expected_shorter_result = get_nibbles_from_bytes(shorter_bytes_suffix);
        let (res_prefix, res_1, res_2) =
            get_common_prefix_nibbles(longer_nibbles, shorter_nibbles).unwrap();
